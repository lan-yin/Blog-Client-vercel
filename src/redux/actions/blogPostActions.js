...
export const getBlogPostsByCategory = (category, pageItems) => async (dispatch) => {
  ...
  try {
    const { data, status } = await axios.get(`${baseURL}/api/blog-posts/${category}/${pageItems}`);
...
export const getBlogPost = (id) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const { data } = await axios.get(`${baseURL}/api/blog-posts/post/${id}`);
...
export const createNewBlogPost = (newPost) => async (dispatch, getState) => {
  ...
  try {
    ...
    await axios.post(`${baseURL}/api/blog-posts`, newPost, config);
...
export const updatePost = (updatedPost) => async (dispatch, getState) => {
  ...
  try {
    ...
    await axios.put(`${baseURL}/api/blog-posts`, updatedPost, config);
...
export const removePost = (_id) => async (dispatch, getState) => {
  ...
  try {
    ...
    const { data } = await axios.delete(`${baseURL}/api/blog-posts/${_id}`, config);
    dispatch(setBlogPostByCategory(data));
